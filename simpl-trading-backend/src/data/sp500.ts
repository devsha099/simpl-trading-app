 ...
 *
 * A STATIC dataset on purpose. Ranking the market by size needs a market-cap
 * feed, and grouping it by sector needs a classification feed; Finnhub's free
 * tier serves neither (its index-constituents endpoint is premium and
 * profile2 is restricted to an allowlist — CLAUDE.md §14). The S&P 500 is
 * public, changes only a handful of times a year, and is a better fit for the
 * brand than a raw market-cap ranking anyway (§1 names index-and-hold
 * believers as a target audience).
 *
 * Same mirrored-static-data pattern as data/usStates.ts and
 * data/financialProfile.ts. All 503 symbols were verified tradable
 * against Alpaca's live asset list when this was generated, so nothing here
 * can 404 the trade screen.
 *
 * Source: datasets/s-and-p-500-companies. Regenerate when the index changes.
 */

export type Sp500Entry = { symbol: string; name: string; sector: string };

/** The 11 GICS sectors present in the index, in display order. */
...
// [symbol, name, sector] tuples rather than objects — 503 rows of
// repeated keys is a lot of bundle for no benefit.
...
export const SP500: Sp500Entry[] = RAW.map(([symbol, name, sector]) => ({ symbol, name, sector }));

export const SP500_SYMBOLS: string[] = SP500.map((e) => e.symbol);

const BY_SYMBOL = new Map(SP500.map((e) => [e.symbol, e]));

export const lookupSp500 = (symbol: string): Sp500Entry | undefined =>
